...
def main() -> int:
    ...
    ap.add_argument("--workspace", type=Path, default=None, help="Directory with job.json (default: parent of -o)")
    ...
    if not args.no_job_check:
        require_stage("ite_breath", ws)

    if not args.chapter_json.is_file():
        if not args.no_job_check:
            mark_failed(ws, "ite_breath", error=f"missing {args.chapter_json}")
    ...
    if should_skip_output(args.out, ["breath_sequences", "pages"], args.force):
        ...
        if not args.no_job_check:
            mark_complete(ws, "ite_breath", output=args.out.name)
        return 0
