...
if [[ $# -gt 0 ]]; then
  python3 "${ROOT_DIR}/scripts/pearl_news_post_to_wp.py" --article "$1" --status draft
else
  python3 "${ROOT_DIR}/scripts/pearl_news_post_to_wp.py" \
    --title "Phoenix Omega draft test" \
    --content "<p>Draft post from the local WordPress wrapper.</p>" \
    --status draft
fi
