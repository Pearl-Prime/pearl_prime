...
def _call_judge(prompt: str, system_prompt: str, cfg: dict) -> str:
    """Call judge model for translation quality assessment (cloud or local)."""
    from scripts.localization.llm_client import call_llm as _cloud_call
    # Use low temperature for consistent judging; judge role selects higher-quality model
    return _cloud_call(system_prompt, prompt, cfg, role="judge", temperature=0.1, max_tokens=1500)
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        nargs="+",
        metavar="LOCALE",
        help="Multiple locales (e.g. ja-JP zh-CN)",
    )
    ...
    if not args.locale and not args.all_locales and not args.locales:
        ap.print_help()
        return 0
    if args.locales and (args.all_locales or args.locale):
        print("Use only one of --locale, --locales, or --all-locales", file=sys.stderr)
        return 2
