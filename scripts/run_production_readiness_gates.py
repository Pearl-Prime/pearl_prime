...
def main() -> int:
    ...
    if lean_best_gate.exists():
        try:
            ...
            lb_detail = str(e)
        if not gate(
            "36. Lean best-book golden integrity (CANONICAL_LEAN_BEST_BOOK sha lock)",
            lb_ok,
            lb_detail,
        ):
            failed += 1
    else:
        gate("36. Lean best-book golden integrity", True, "gate script not present; skip", skip=True)
    ...
    for gate_id, script_name, title, extra_env in (
        (
            ...
            "37. G-CLAIM / Q-ENFORCE-02 acceptance claim language",
            {"PYTHONPATH": str(REPO_ROOT / "scripts" / "ci") + os.pathsep + str(REPO_ROOT)},
        ),
        (
            "36",
            "check_catalog_manifest_acceptance_layer.py",
            "38. G-LAYER catalog manifest acceptance_layer",
            {"PYTHONPATH": str(REPO_ROOT)},
        ),
        (
            "37",
            "check_catalog_ship_wrap_defect4.py",
            "39. G-WRAP + G-DEF4 catalog ship detectors (integrity)",
            {"PYTHONPATH": str(REPO_ROOT)},
        ),
    ):
    ...
    for gate_id, script_name, title, extra_env in (
        (
            ...
            "40. G-F1H templated-cluster HARD_FAIL escalation (integrity)",
            {"PYTHONPATH": str(REPO_ROOT)},
        ),
        (
            "39",
            "check_orient_ch1_scene_anchor.py",
            "41. G-ORIENT Ch1 body/scene anchor lexicon (integrity; WARN-class)",
            {"PYTHONPATH": str(REPO_ROOT)},
        ),
        (
            "40",
            "check_accent_supply_preflight.py",
            "42. G-ACCENT weekly accent-fill preflight matrix (report-only here)",
            {"PYTHONPATH": str(REPO_ROOT)},
        ),
    ):
        script = REPO_ROOT / "scripts" / "ci" / script_name
