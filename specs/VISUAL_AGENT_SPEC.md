...
> **Render-authority note (provenance, 2026-05-29):** this is the SpiritualTech-lineage visual spec (FLUX/SDXL prompt planning, LoRA identity). The **current production render pipeline** is V5.1 — `docs/specs/MANGA_V5_LAYERED_ARCHITECTURE.md` (Qwen-Image-Layered) + `docs/specs/MANGA_CONTINUITY_STATE_SPEC.md` + PuLID-FLUX identity. The two lineages are not yet cross-reconciled; confirm which render path applies (operator-tier) before production use. Prompt approach here aligns with `docs/COMFYUI_FLUX_MANGA_PROMPTING_RESEARCH_2026-04-29.md`. **Reconciliation decision (2026-05-29):** `docs/MANGA_RENDER_LINEAGE_DECISION_2026-05-29.md` recommends the V4/V5 contract + `continuity_state` chain as canonical for render and this spec's prompt-planning as **superseded for the layered-render approach** (operator ratification pending; spec retained as experiment-of-record / non-layered fallback).
