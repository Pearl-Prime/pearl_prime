...
def test_list_candidates_requires_atom_bank(tmp_path: Path) -> None:
    ...
    with mock.patch(
        "scripts.qa.run_random_2h_book_with_trace._tuple_is_viable",
        return_value=True,
    ):
...
def test_list_candidates_one_per_persona_topic(tmp_path: Path) -> None:
    ...
    with mock.patch(
        ...
        cands = list_candidates(repo_root=tmp_path)
    assert len(cands) == 1
    # sorted glob → comparison before overwhelm when both viable
    assert cands[0].engine == "comparison"


def test_list_candidates_skips_nonviable_engine(tmp_path: Path) -> None:
    """First alphabetical engine may be NO_BINDING; pick next viable engine."""
    _write_arc(tmp_path, "p1", "t1", "comparison", "F006")
    _write_arc(tmp_path, "p1", "t1", "false_alarm", "F006")
    _touch_canonical(tmp_path, "p1", "t1")

    def _viable(persona, topic, engine, format_id, *, repo_root, **_kwargs):
        return engine == "false_alarm"
    ...
    with mock.patch(
        ...
        cands = list_candidates(repo_root=tmp_path)
    assert len(cands) == 1
    assert cands[0].engine == "false_alarm"
