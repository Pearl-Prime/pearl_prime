...
def test_search_videos_passes_ctqc_kwargs():
    captured: dict = {}
    ...
    client.search_videos(
        ...
        min_duration=3,
        max_duration=12,
        required_keywords="anxious",
        filtered_keywords="festival,concert",
        extended="keywords,durationMs",
    )
    assert captured["url"].endswith("/api/v2/videos/search")
    assert captured["params"]["min_duration"] == 3
    assert captured["params"]["required_keywords"] == "anxious"
    assert captured["params"]["filtered_keywords"] == "festival,concert"
    assert captured["params"]["extended"] == "keywords,durationMs"


def test_search_audio_shared_client():
    captured: dict = {}
    ...
    assert captured["url"].endswith("/api/v2/audio/search")
    assert captured["params"]["has_vocals"] == "false"
    assert captured["params"]["min_bpm"] == 90


def test_keyword_tagger_and_provider_smoke_mocked(tmp_path: Path):
    from scripts.storyblocks.video_provider import smoke_ingest

    store = LicenseStore(bank_root=tmp_path / "bank", index_path=tmp_path / "idx.jsonl")
    ledger = MauLedger(path=tmp_path / "mau.jsonl", hard_cap=104)
    tag_store = tmp_path / "asset_keywords.tsv"
    # Seed header so upsert has schema
    tag_store.write_text(
        "asset_path\ttopic_folder\tkeywords\tmetaphor\ttitle\n", encoding="utf-8"
    )
    ...
    hd_body = SimpleNamespace(status_code=200, content=b"\x00FAKEHD")
    ...
    result = smoke_ingest(
        ...
        mock_results=[
            {
                "id": "SBV-42",
                "title": "Anxious woman at office desk with phone",
                "keywords": ["anxious", "office", "phone", "nervous"],
                "duration": 5,
                "hasTalentReleased": True,
            }
        ],
    )
    assert result.stock_id == "SBV-42"
    assert result.mau_distinct_count == 1
    assert Path(result.local_uri).exists()
    assert "anxious" in result.tag_row["keywords"]
    text = tag_store.read_text(encoding="utf-8")
    assert "SBV-42" in text or "anxiety" in text
    # Consumer guard blocks unlicensed sibling
    with pytest.raises(UnlicensedStoryblocksAssetError):
        assert_storyblocks_licensed_for_consumer(
            {
                "source_provider": "storyblocks",
                "storyblocks_stock_id": "SBV-OTHER",
    ...
    assert_storyblocks_licensed_for_consumer(
        {
            "source_provider": "storyblocks",
            "storyblocks_stock_id": "SBV-42",
            "work_unit_id": result.work_unit_id,
            "local_uri": result.local_uri,
        },
        work_unit_id=result.work_unit_id,
        license_store=store,
    )
...
def test_stock_image_bank_storyblocks_is_preview_only(tmp_path: Path):
    from scripts.brand.build_stock_image_bank import StockImageBankError, main, provider_policy

    pol = provider_policy("storyblocks")
    assert pol.storage_mode == "metadata_only_preview"
    cfg = tmp_path / "topics.yaml"
    cfg.write_text(
        "collection_id: t\ntopics:\n  anxiety:\n    metaphors: [anxious phone]\n",
        encoding="utf-8",
    )
    with pytest.raises(StockImageBankError, match="forbidden"):
        main(
            [
                "--brand-id",
