...
**Purpose:** After Pearl Prime recovery PRs **#67–#76**, runtime on `main` is **honest**: hollow banks and missing location profiles fail visibly instead of silently drifting. This spec audits **remaining content debt** under `atoms/gen_z_professionals/`, **teacher banks**, and **location profiles**, then defines a **scoped repair lane** (prose + YAML only — **no runtime code** in this lane).
...
**Status:** Dev spec only — **no atom content** and **no code changes** are part of this document’s own deliverable.
...
### Runtime / config drift flag (separate corrective)

**Not** source-bank debt: [config/catalog_planning/teacher_persona_matrix.yaml](../config/catalog_planning/teacher_persona_matrix.yaml) lists `coastal_california` under `ahjan.preferred_locales`, but [config/localization/render_location_profiles.yaml](../config/localization/render_location_profiles.yaml) defines only **`nyc_metro`** and **`nyc_grand_central`**. A run with `--location coastal_california` will hit **missing profile** behavior at render. **Recommendation:** open a small **runtime/config alignment** task (add profile or remove preference) — **do not** bury this inside atom repair PRs.
...
1. **`coastal_california`** — align with `teacher_persona_matrix` ahjan preference **or** drop preference in config task (see §1 flag).
2. **`generic_us_urban`** — non-coastal, non-NYC (e.g. Midwest/Sunbelt) with distinct tokens (avoid cloning NYC strings).
...
5. **Location MVP** — **P1** (`coastal_california` + `generic_us_urban` or config alignment PR first).
...
| Scope | Definition |
|-------|------------|
| **MVP “production content-ready”** | overthinking SCENE + all hollow/thin COMPRESSION + sleep HOOK strengthened; **either** `coastal_california` profile **or** matrix fix; teacher banks unchanged **if** product accepts persona-fallback Teacher Mode **or** ahjan gets minimal HOOK/SCENE/REFLECTION/INTEGRATION set for one demo arc. |
...
| **PR-S5** | `config/localization/render_location_profiles.yaml` — add MVP profiles + alias lists; **no Python** | Pearl_Dev |
...
**Suggested count:** **8** PRs; can merge S3+S4 or split S8 by topic for reviewability.
...
### 7.4 Config alignment (separate small PR — not PR-S*)

- `config/catalog_planning/teacher_persona_matrix.yaml` (**modify**) *or* profiles added — **do not** leave dangling `coastal_california` preference without profile.
...
- Optional: add fixture test path later — **not** in this spec’s write scope.
...
2. **Do not** change Python/renderer/planner in this lane.
...
| Location profiles in YAML | **2** (both NYC) |

### 11.2 PR slices

- **8** proposed ordered slices (**PR-S1** … **PR-S8**); merge/split allowed for review size.
...
- **Config/runtime drift:** `coastal_california` in matrix without profile is a **product blocker** for that locale until resolved (separate tiny PR).
