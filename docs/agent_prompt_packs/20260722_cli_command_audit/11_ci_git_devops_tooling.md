...
# Lane 11 — CLI Audit: Ci Git Devops Tooling
...
STARTUP_RECEIPT
AGENT:              Pearl_DevOps
TASK:               CLI command audit + reference doc — Ci Git Devops Tooling
PROJECT_ID:         proj_cli_command_audit_20260722
SUBSYSTEM:          pearl_devops
AUTHORITY_DOCS:     docs/GITHUB_OPERATIONS_FRAMEWORK.md;docs/GITHUB_GOVERNANCE.md;docs/BRANCH_PROTECTION_REQUIREMENTS.md;CLAUDE.md
READ_PATH_COMPLETE: yes
WRITE_SCOPE:        docs/cli_reference/ci_git_devops_tooling.md; docs/cli_reference/findings/ci_git_devops_tooling_FINDINGS.tsv; one row in artifacts/coordination/ACTIVE_WORKSTREAMS.tsv
...
PROVENANCE:         research: NONE (documentation/audit task); documents: docs/GITHUB_OPERATIONS_FRAMEWORK.md;docs/GITHUB_GOVERNANCE.md;docs/BRANCH_PROTECTION_REQUIREMENTS.md;CLAUDE.md; builds_on: N/A; inventory: UNCHANGED — read-only audit, zero code changes
BLOCKERS:           none known
READY_STATUS:       ready
```
...
```
git fetch origin
git checkout -b agent/cli-audit-ci_git_devops_tooling-20260722 origin/main
gh pr list --state open --search "cli_reference ci_git_devops_tooling"
```
...
Files under: `scripts/ci/ (101 files), scripts/git/ (6 files), scripts/audit/ (8 files), scripts/observability/ (6 files)`

## Lane-specific caution

scripts/ci/ is the largest infra directory (101 files) and most are CI GATES wired into .github/workflows/*.yml — grep every workflow file for each script's basename before any DELETE-CANDIDATE recommendation; a gate script referenced in a required status check (Drift detectors, LLM policy enforcement) must be marked KEEP regardless of how obscure it looks. scripts/git/ includes push_guard.py and health_check.sh-adjacent tooling named in this repo's CLAUDE.md Mandatory Preflight — these are KEEP by construction.
...
1. `docs/cli_reference/ci_git_devops_tooling.md` — one row/section per command: path, one-line
...
2. `docs/cli_reference/findings/ci_git_devops_tooling_FINDINGS.tsv` — machine-readable, columns:
...
Open a PR titled `docs(cli-audit): ci_git_devops_tooling CLI reference + findings`. Merge it yourself
...
CLOSEOUT_RECEIPT
AGENT:          Pearl_DevOps
TASK:           CLI command audit — Ci Git Devops Tooling
COMMIT_SHA:     <full merge SHA>
FILES_WRITTEN:  docs/cli_reference/ci_git_devops_tooling.md; docs/cli_reference/findings/ci_git_devops_tooling_FINDINGS.tsv
FILES_READ:     <authority docs actually opened>
PROVENANCE:     research: NONE; documents: docs/GITHUB_OPERATIONS_FRAMEWORK.md;docs/GITHUB_GOVERNANCE.md;docs/BRANCH_PROTECTION_REQUIREMENTS.md;CLAUDE.md; builds_on: N/A; inventory: UNCHANGED
STATUS:         completed | partial | blocked
HANDOFF_TO:     Pearl_PM (synthesis lane 13)
NEXT_ACTION:    <if partial/blocked: exact resume point>
```

Signal token: `CLI_AUDIT_CI_GIT_DEVOPS_TOOLING_COMPLETE: <n_commands> commands, <n_flagged> flagged, SHA <full_sha>`
