...
# Lane 12 — CLI Audit: Editorial Teacher Research
...
STARTUP_RECEIPT
AGENT:              Pearl_Editor
TASK:               CLI command audit + reference doc — Editorial Teacher Research
PROJECT_ID:         proj_cli_command_audit_20260722
SUBSYSTEM:          teacher_mode
AUTHORITY_DOCS:     docs/SYSTEMS_V4.md;specs/PHOENIX_V4_5_WRITER_SPEC.md
READ_PATH_COMPLETE: yes
WRITE_SCOPE:        docs/cli_reference/editorial_teacher_research.md; docs/cli_reference/findings/editorial_teacher_research_FINDINGS.tsv; one row in artifacts/coordination/ACTIVE_WORKSTREAMS.tsv
...
PROVENANCE:         research: NONE (documentation/audit task); documents: docs/SYSTEMS_V4.md;specs/PHOENIX_V4_5_WRITER_SPEC.md; builds_on: N/A; inventory: UNCHANGED — read-only audit, zero code changes
BLOCKERS:           none known
READY_STATUS:       ready
```
...
```
git fetch origin
git checkout -b agent/cli-audit-editorial_teacher_research-20260722 origin/main
gh pr list --state open --search "cli_reference editorial_teacher_research"
```
...
Files under: `tools/teacher_mining/ (9 files), scripts/research/ (9 files), scripts/ml_editorial/ (6 files)`

## Lane-specific caution

Cross-check tools/teacher_mining/ against docs/PEARL_PRIME_BESTSELLER_ACCEPTANCE_SCORECARD.md and the LLM Tier Policy in CLAUDE.md — any script here that calls a paid LLM API directly (rather than Gemma/Qwen via Ollama or routing through Claude Code) is a POLICY VIOLATION, not just a duplication finding; flag it as CRITICAL in the findings TSV and note it separately from the KEEP/DELETE recommendation.
...
1. `docs/cli_reference/editorial_teacher_research.md` — one row/section per command: path, one-line
...
2. `docs/cli_reference/findings/editorial_teacher_research_FINDINGS.tsv` — machine-readable, columns:
...
Open a PR titled `docs(cli-audit): editorial_teacher_research CLI reference + findings`. Merge it yourself
...
CLOSEOUT_RECEIPT
AGENT:          Pearl_Editor
TASK:           CLI command audit — Editorial Teacher Research
COMMIT_SHA:     <full merge SHA>
FILES_WRITTEN:  docs/cli_reference/editorial_teacher_research.md; docs/cli_reference/findings/editorial_teacher_research_FINDINGS.tsv
FILES_READ:     <authority docs actually opened>
PROVENANCE:     research: NONE; documents: docs/SYSTEMS_V4.md;specs/PHOENIX_V4_5_WRITER_SPEC.md; builds_on: N/A; inventory: UNCHANGED
STATUS:         completed | partial | blocked
HANDOFF_TO:     Pearl_PM (synthesis lane 13)
NEXT_ACTION:    <if partial/blocked: exact resume point>
```

Signal token: `CLI_AUDIT_EDITORIAL_TEACHER_RESEARCH_COMPLETE: <n_commands> commands, <n_flagged> flagged, SHA <full_sha>`
