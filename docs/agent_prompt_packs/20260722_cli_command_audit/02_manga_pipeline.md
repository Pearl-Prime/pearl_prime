...
# Lane 02 — CLI Audit: Manga Pipeline
...
STARTUP_RECEIPT
AGENT:              Pearl_Dev
TASK:               CLI command audit + reference doc — Manga Pipeline
PROJECT_ID:         proj_cli_command_audit_20260722
SUBSYSTEM:          manga_pipeline
AUTHORITY_DOCS:     specs/AI_MANGA_PIPELINE_SUMMARY.md;docs/MANGA_IMPLEMENTATION_OUTLINE.md
READ_PATH_COMPLETE: yes
WRITE_SCOPE:        docs/cli_reference/manga_pipeline.md; docs/cli_reference/findings/manga_pipeline_FINDINGS.tsv; one row in artifacts/coordination/ACTIVE_WORKSTREAMS.tsv
...
PROVENANCE:         research: NONE (documentation/audit task); documents: specs/AI_MANGA_PIPELINE_SUMMARY.md;docs/MANGA_IMPLEMENTATION_OUTLINE.md; builds_on: N/A; inventory: UNCHANGED — read-only audit, zero code changes
BLOCKERS:           none known
READY_STATUS:       ready
```
...
```
git fetch origin
git checkout -b agent/cli-audit-manga_pipeline-20260722 origin/main
gh pr list --state open --search "cli_reference manga_pipeline"
```
...
Files under: `scripts/manga/ (117 CLI-shaped files — the largest single lane)`

## Lane-specific caution

Read docs/specs/MANGA_100PCT_PRODUCTION_ROADMAP_2026-07-03.md and artifacts/qa/MANGA_VISION_CONFORMANCE_AUDIT_2026-07-03.md first (CLAUDE.md Manga Vision-Conformance Doctrine) — label every command's actual layer (CODE-WIRED vs EXECUTED-REAL etc.), do not assume a CLI existing means it's working. This directory is large; batch it in groups of ~20 files and check in progress rather than holding all 117 in context at once.
...
1. `docs/cli_reference/manga_pipeline.md` — one row/section per command: path, one-line
...
2. `docs/cli_reference/findings/manga_pipeline_FINDINGS.tsv` — machine-readable, columns:
...
Open a PR titled `docs(cli-audit): manga_pipeline CLI reference + findings`. Merge it yourself
...
CLOSEOUT_RECEIPT
AGENT:          Pearl_Dev
TASK:           CLI command audit — Manga Pipeline
COMMIT_SHA:     <full merge SHA>
FILES_WRITTEN:  docs/cli_reference/manga_pipeline.md; docs/cli_reference/findings/manga_pipeline_FINDINGS.tsv
FILES_READ:     <authority docs actually opened>
PROVENANCE:     research: NONE; documents: specs/AI_MANGA_PIPELINE_SUMMARY.md;docs/MANGA_IMPLEMENTATION_OUTLINE.md; builds_on: N/A; inventory: UNCHANGED
STATUS:         completed | partial | blocked
HANDOFF_TO:     Pearl_PM (synthesis lane 13)
NEXT_ACTION:    <if partial/blocked: exact resume point>
```

Signal token: `CLI_AUDIT_MANGA_PIPELINE_COMPLETE: <n_commands> commands, <n_flagged> flagged, SHA <full_sha>`
