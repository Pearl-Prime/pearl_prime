...
STARTUP_RECEIPT
AGENT:              Pearl_Dev
TASK:               CLI command audit + reference doc — Qa Gates
PROJECT_ID:         proj_cli_command_audit_20260722
SUBSYSTEM:          core_pipeline
AUTHORITY_DOCS:     docs/PEARL_PRIME_WHOLE_WORKFLOW_HARDENING_SPEC.md;specs/PHOENIX_V4_5_WRITER_SPEC.md
READ_PATH_COMPLETE: yes
WRITE_SCOPE:        docs/cli_reference/qa_gates.md; docs/cli_reference/findings/qa_gates_FINDINGS.tsv; one row in artifacts/coordination/ACTIVE_WORKSTREAMS.tsv
...
PROVENANCE:         research: NONE (documentation/audit task); documents: docs/PEARL_PRIME_WHOLE_WORKFLOW_HARDENING_SPEC.md;specs/PHOENIX_V4_5_WRITER_SPEC.md; builds_on: N/A; inventory: UNCHANGED — read-only audit, zero code changes
BLOCKERS:           none known
READY_STATUS:       ready
```
...
```
git fetch origin
git checkout -b agent/cli-audit-qa_gates-20260722 origin/main
gh pr list --state open --search "cli_reference qa_gates"
```
...
Files under: `scripts/qa/ (38 files), phoenix_v4/qa/ (9 files), artifacts/qa/ (9 files — only the .py entries, not report artifacts)`

## Lane-specific caution

Many of these are production readiness GATES wired into scripts/run_production_readiness_gates.py or the Drift detectors CI workflow (.github/workflows/drift-detectors.yml) — grep both before flagging anything as unused; a gate with zero grep hits outside its own file is a stronger signal than one that's imported elsewhere. Never recommend deleting a gate without noting whether it is CI-required.
...
1. `docs/cli_reference/qa_gates.md` — one row/section per command: path, one-line
...
2. `docs/cli_reference/findings/qa_gates_FINDINGS.tsv` — machine-readable, columns:
...
Open a PR titled `docs(cli-audit): qa_gates CLI reference + findings`. Merge it yourself
...
CLOSEOUT_RECEIPT
AGENT:          Pearl_Dev
TASK:           CLI command audit — Qa Gates
COMMIT_SHA:     <full merge SHA>
FILES_WRITTEN:  docs/cli_reference/qa_gates.md; docs/cli_reference/findings/qa_gates_FINDINGS.tsv
FILES_READ:     <authority docs actually opened>
PROVENANCE:     research: NONE; documents: docs/PEARL_PRIME_WHOLE_WORKFLOW_HARDENING_SPEC.md;specs/PHOENIX_V4_5_WRITER_SPEC.md; builds_on: N/A; inventory: UNCHANGED
STATUS:         completed | partial | blocked
HANDOFF_TO:     Pearl_PM (synthesis lane 13)
NEXT_ACTION:    <if partial/blocked: exact resume point>
```

Signal token: `CLI_AUDIT_QA_GATES_COMPLETE: <n_commands> commands, <n_flagged> flagged, SHA <full_sha>`
