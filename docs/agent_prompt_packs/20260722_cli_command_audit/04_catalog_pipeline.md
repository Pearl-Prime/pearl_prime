...
# Lane 04 — CLI Audit: Catalog Pipeline
...
STARTUP_RECEIPT
AGENT:              Pearl_Prime
TASK:               CLI command audit + reference doc — Catalog Pipeline
PROJECT_ID:         proj_cli_command_audit_20260722
SUBSYSTEM:          core_pipeline
AUTHORITY_DOCS:     specs/PHOENIX_ARC_FIRST_CANONICAL_SPEC.md;docs/PEARL_PRIME_BOOK_SYSTEM_CANONICAL.md
READ_PATH_COMPLETE: yes
WRITE_SCOPE:        docs/cli_reference/catalog_pipeline.md; docs/cli_reference/findings/catalog_pipeline_FINDINGS.tsv; one row in artifacts/coordination/ACTIVE_WORKSTREAMS.tsv
...
PROVENANCE:         research: NONE (documentation/audit task); documents: specs/PHOENIX_ARC_FIRST_CANONICAL_SPEC.md;docs/PEARL_PRIME_BOOK_SYSTEM_CANONICAL.md; builds_on: N/A; inventory: UNCHANGED — read-only audit, zero code changes
BLOCKERS:           none known
READY_STATUS:       ready
```
...
```
git fetch origin
git checkout -b agent/cli-audit-catalog_pipeline-20260722 origin/main
gh pr list --state open --search "cli_reference catalog_pipeline"
```
...
Files under: `scripts/catalog/ (30 files), scripts/catalog_visibility/ (9 files)`

## Lane-specific caution

Catalog scale-up is an active lane (project_catalog_scaleup_lane memory) — cross-check any script that looks like a per-brand CSV-to-YAML driver against that lane before flagging as duplicate; it may be the active canonical tool.
...
1. `docs/cli_reference/catalog_pipeline.md` — one row/section per command: path, one-line
...
2. `docs/cli_reference/findings/catalog_pipeline_FINDINGS.tsv` — machine-readable, columns:
...
Open a PR titled `docs(cli-audit): catalog_pipeline CLI reference + findings`. Merge it yourself
...
CLOSEOUT_RECEIPT
AGENT:          Pearl_Prime
TASK:           CLI command audit — Catalog Pipeline
COMMIT_SHA:     <full merge SHA>
FILES_WRITTEN:  docs/cli_reference/catalog_pipeline.md; docs/cli_reference/findings/catalog_pipeline_FINDINGS.tsv
FILES_READ:     <authority docs actually opened>
PROVENANCE:     research: NONE; documents: specs/PHOENIX_ARC_FIRST_CANONICAL_SPEC.md;docs/PEARL_PRIME_BOOK_SYSTEM_CANONICAL.md; builds_on: N/A; inventory: UNCHANGED
STATUS:         completed | partial | blocked
HANDOFF_TO:     Pearl_PM (synthesis lane 13)
NEXT_ACTION:    <if partial/blocked: exact resume point>
```

Signal token: `CLI_AUDIT_CATALOG_PIPELINE_COMPLETE: <n_commands> commands, <n_flagged> flagged, SHA <full_sha>`
