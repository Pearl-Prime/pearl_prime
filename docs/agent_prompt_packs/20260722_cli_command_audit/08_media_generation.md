...
# Lane 08 — CLI Audit: Media Generation
...
STARTUP_RECEIPT
AGENT:              Pearl_Dev
TASK:               CLI command audit + reference doc — Media Generation
PROJECT_ID:         proj_cli_command_audit_20260722
SUBSYSTEM:          music_mode
AUTHORITY_DOCS:     docs/PEARL_ARCHITECT_STATE.md MUSIC-MODE-V1-01;docs/PODCAST_PIPELINE_INTEGRATION_SPEC.md;docs/ROBUST_AGENT_PROTOCOL.md
READ_PATH_COMPLETE: yes
WRITE_SCOPE:        docs/cli_reference/media_generation.md; docs/cli_reference/findings/media_generation_FINDINGS.tsv; one row in artifacts/coordination/ACTIVE_WORKSTREAMS.tsv
...
PROVENANCE:         research: NONE (documentation/audit task); documents: docs/PEARL_ARCHITECT_STATE.md MUSIC-MODE-V1-01;docs/PODCAST_PIPELINE_INTEGRATION_SPEC.md;docs/ROBUST_AGENT_PROTOCOL.md; builds_on: N/A; inventory: UNCHANGED — read-only audit, zero code changes
BLOCKERS:           none known
READY_STATUS:       ready
```
...
```
git fetch origin
git checkout -b agent/cli-audit-media_generation-20260722 origin/main
gh pr list --state open --search "cli_reference media_generation"
```
...
Files under: `scripts/image_generation/ (15 files), scripts/music/ (7 files), scripts/podcast/ (6 files)`

## Lane-specific caution

scripts/image_generation/ scripts that call ComfyUI/RunComfy directly (bypassing pscli queue) are a RAP-compliance flag, not just a duplication flag — note this separately in the findings TSV's notes column, cross-reference scripts/ci/check_rap_compliance.py's known bypass patterns.
...
1. `docs/cli_reference/media_generation.md` — one row/section per command: path, one-line
...
2. `docs/cli_reference/findings/media_generation_FINDINGS.tsv` — machine-readable, columns:
...
Open a PR titled `docs(cli-audit): media_generation CLI reference + findings`. Merge it yourself
...
CLOSEOUT_RECEIPT
AGENT:          Pearl_Dev
TASK:           CLI command audit — Media Generation
COMMIT_SHA:     <full merge SHA>
FILES_WRITTEN:  docs/cli_reference/media_generation.md; docs/cli_reference/findings/media_generation_FINDINGS.tsv
FILES_READ:     <authority docs actually opened>
PROVENANCE:     research: NONE; documents: docs/PEARL_ARCHITECT_STATE.md MUSIC-MODE-V1-01;docs/PODCAST_PIPELINE_INTEGRATION_SPEC.md;docs/ROBUST_AGENT_PROTOCOL.md; builds_on: N/A; inventory: UNCHANGED
STATUS:         completed | partial | blocked
HANDOFF_TO:     Pearl_PM (synthesis lane 13)
NEXT_ACTION:    <if partial/blocked: exact resume point>
```

Signal token: `CLI_AUDIT_MEDIA_GENERATION_COMPLETE: <n_commands> commands, <n_flagged> flagged, SHA <full_sha>`
