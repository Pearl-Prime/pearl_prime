...
# Lane 06 — CLI Audit: Localization
...
TASK:               CLI command audit + reference doc — Localization
PROJECT_ID:         proj_cli_command_audit_20260722
SUBSYSTEM:          translation
AUTHORITY_DOCS:     config/localization/quality_contracts/README.md;docs/DOCS_INDEX.md
READ_PATH_COMPLETE: yes
WRITE_SCOPE:        docs/cli_reference/localization.md; docs/cli_reference/findings/localization_FINDINGS.tsv; one row in artifacts/coordination/ACTIVE_WORKSTREAMS.tsv
...
PROVENANCE:         research: NONE (documentation/audit task); documents: config/localization/quality_contracts/README.md;docs/DOCS_INDEX.md; builds_on: N/A; inventory: UNCHANGED — read-only audit, zero code changes
BLOCKERS:           none known
READY_STATUS:       ready
```
...
```
git fetch origin
git checkout -b agent/cli-audit-localization-20260722 origin/main
gh pr list --state open --search "cli_reference localization"
```
...
Files under: `scripts/localization/ (18 files)`

## Lane-specific caution

Cross-check against docs/TRANSLATE_QWEN_PIPELINE_CLI.md (the existing canonical translate CLI doc) — extend/correct it rather than treating this lane as a from-scratch inventory if it already covers part of this directory.
...
1. `docs/cli_reference/localization.md` — one row/section per command: path, one-line
...
2. `docs/cli_reference/findings/localization_FINDINGS.tsv` — machine-readable, columns:
...
Open a PR titled `docs(cli-audit): localization CLI reference + findings`. Merge it yourself
...
CLOSEOUT_RECEIPT
AGENT:          Pearl_Localization
TASK:           CLI command audit — Localization
COMMIT_SHA:     <full merge SHA>
FILES_WRITTEN:  docs/cli_reference/localization.md; docs/cli_reference/findings/localization_FINDINGS.tsv
FILES_READ:     <authority docs actually opened>
PROVENANCE:     research: NONE; documents: config/localization/quality_contracts/README.md;docs/DOCS_INDEX.md; builds_on: N/A; inventory: UNCHANGED
STATUS:         completed | partial | blocked
HANDOFF_TO:     Pearl_PM (synthesis lane 13)
NEXT_ACTION:    <if partial/blocked: exact resume point>
```

Signal token: `CLI_AUDIT_LOCALIZATION_COMPLETE: <n_commands> commands, <n_flagged> flagged, SHA <full_sha>`
