...
# Lane 05 — CLI Audit: Publish Release
...
STARTUP_RECEIPT
AGENT:              Pearl_DevOps
TASK:               CLI command audit + reference doc — Publish Release
PROJECT_ID:         proj_cli_command_audit_20260722
SUBSYSTEM:          pearl_devops
AUTHORITY_DOCS:     docs/GITHUB_OPERATIONS_FRAMEWORK.md;docs/GITHUB_GOVERNANCE.md;docs/BRANCH_PROTECTION_REQUIREMENTS.md
READ_PATH_COMPLETE: yes
WRITE_SCOPE:        docs/cli_reference/publish_release.md; docs/cli_reference/findings/publish_release_FINDINGS.tsv; one row in artifacts/coordination/ACTIVE_WORKSTREAMS.tsv
...
PROVENANCE:         research: NONE (documentation/audit task); documents: docs/GITHUB_OPERATIONS_FRAMEWORK.md;docs/GITHUB_GOVERNANCE.md;docs/BRANCH_PROTECTION_REQUIREMENTS.md; builds_on: N/A; inventory: UNCHANGED — read-only audit, zero code changes
BLOCKERS:           none known
READY_STATUS:       ready
```
...
```
git fetch origin
git checkout -b agent/cli-audit-publish_release-20260722 origin/main
gh pr list --state open --search "cli_reference publish_release"
```
...
Files under: `scripts/publish/ (17 files), scripts/release/ (15 files)`

## Lane-specific caution

Some of these may be wired into .github/workflows/ — grep the workflows directory for each script's filename before recommending DELETE-CANDIDATE; a script invoked only from CI is not 'unused' even with zero other callers.
...
1. `docs/cli_reference/publish_release.md` — one row/section per command: path, one-line
...
2. `docs/cli_reference/findings/publish_release_FINDINGS.tsv` — machine-readable, columns:
...
Open a PR titled `docs(cli-audit): publish_release CLI reference + findings`. Merge it yourself
...
CLOSEOUT_RECEIPT
AGENT:          Pearl_DevOps
TASK:           CLI command audit — Publish Release
COMMIT_SHA:     <full merge SHA>
FILES_WRITTEN:  docs/cli_reference/publish_release.md; docs/cli_reference/findings/publish_release_FINDINGS.tsv
FILES_READ:     <authority docs actually opened>
PROVENANCE:     research: NONE; documents: docs/GITHUB_OPERATIONS_FRAMEWORK.md;docs/GITHUB_GOVERNANCE.md;docs/BRANCH_PROTECTION_REQUIREMENTS.md; builds_on: N/A; inventory: UNCHANGED
STATUS:         completed | partial | blocked
HANDOFF_TO:     Pearl_PM (synthesis lane 13)
NEXT_ACTION:    <if partial/blocked: exact resume point>
```

Signal token: `CLI_AUDIT_PUBLISH_RELEASE_COMPLETE: <n_commands> commands, <n_flagged> flagged, SHA <full_sha>`
