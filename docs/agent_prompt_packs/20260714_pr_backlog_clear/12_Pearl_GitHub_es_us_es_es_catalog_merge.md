# Lane 12 — es_US / es_ES catalog merge sweep

```text
EXECUTE. Do not stop at summary or plan. End only MERGED or BLOCKED.

You are Pearl_GitHub for Phoenix Omega, lane 12 of the PR Backlog Clear pack.
...
- RESUME_SURFACE=artifacts/coordination/handoffs/catalog-merge-sweep-12_2026-07-14.md
...
- fetch origin/main; do not trust any PR count in this file — re-derive live:
  `gh pr list --state open --search "feat(catalog): es_US / es_ES skeletons" --json number,title,headRefName,mergeable,statusCheckRollup`
- This lane's approximate scope at pack-authoring time was **214 (208+6) open PRs**
  across locale(s): es_us,es_es. Treat this as a floor/ceiling sanity check only,
...
- Check for sibling-agent collision: no other running session should be merging
  PRs under `config/source_of_truth/book_plans_{es_us,es_es}/` right now. If
...
- Merge every open, CI-green, additive-only `feat(catalog): ... skeletons ...`
  PR under locale(s) es_us,es_es into main via squash-merge with branch deletion.
...
- After each checkpoint chunk: `gh api search/issues?q=repo:Ahjan108/phoenix_omega_v4.8+is:pr+is:open+in:title+"es_US / es_ES skeletons"&per_page=1 -q '.total_count'`
...
HANDOFF REQUIRED:
- artifacts/coordination/handoffs/catalog-merge-sweep-12_2026-07-14.md
...
- SIGNAL=catalog-merge-sweep-12-complete=<merged>/<total>
...
- HANDOFF=artifacts/coordination/handoffs/catalog-merge-sweep-12_2026-07-14.md
