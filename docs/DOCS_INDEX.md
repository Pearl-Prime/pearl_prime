...
| **START HERE — Integration credentials (canonical)** | [docs/INTEGRATION_CREDENTIALS_REGISTRY.md](./INTEGRATION_CREDENTIALS_REGISTRY.md) — use this before local intake or messaging setup; then run `python3 scripts/ci/check_integration_env.py` (or `--json`) for env wiring. |
