...
| **Find all integration credentials (canonical registry)** | [docs/INTEGRATION_CREDENTIALS_REGISTRY.md](./INTEGRATION_CREDENTIALS_REGISTRY.md) — single source of truth for every API key, token, and secret; per-service env var names, consumers, setup links, and status. Run `python3 scripts/ci/check_integration_env.py` to check what's wired. |
