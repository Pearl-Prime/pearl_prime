...
- [specs/ACCENT_BEATS_SYSTEM_SPEC.md](../specs/ACCENT_BEATS_SYSTEM_SPEC.md) — **CODE-WIRED (partial live).** Planner-placed sparse accents (quote, encouragement, reflection question, troubleshooting, cited evidence, author commentary, external story, wisdom essence). **AFFIRMATION is NOT LIVE** (TODO mantras only; excluded from runtime classes). Framing-inside-atom; witness-register AUTHOR_COMMENTARY §12; PERMISSION reuse; QUOTE + CITED_EVIDENCE banks; GATE: `check_accent_flagship_truth.py` + `flagship-read-approved`
