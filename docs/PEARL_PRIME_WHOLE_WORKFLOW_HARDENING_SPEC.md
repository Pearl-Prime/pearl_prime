...
3. **NYC was not first-class** — the CLI has no explicit location input; rendered prose used generic fallbacks from [phoenix_v4/rendering/book_renderer.py](../phoenix_v4/rendering/book_renderer.py), producing repeated lines like `gray light through the window`.
