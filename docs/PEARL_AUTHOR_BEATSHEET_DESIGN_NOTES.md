...
- **v1.2** (2026-05-26): Multi-character generator extension shipped (joint with OPDs 147+148). New archetypes: `secondary_character_face_close` (OPD-149, parametrized via `subject_actor`) and `typographic_caption_card` (OPD-150, META cluster). Promoted from declared-but-unimplemented (ep_002 surfaced): `walking_in_thought_medium`, `miyazaki_ma_pause`, `window_light_threshold`. OPEN-5 and OPEN-7 resolved.
...
### §3.1 The first parametrized archetype (OPD-149)

`secondary_character_face_close` is the **first parametrized archetype**: it requires a `subject_actor: <character_id>` field per beat that binds the on-frame focal character. This is also the first archetype that diverges from the V1 "single stage_character" assumption. The generator's multi-character extension (this PR, joint with OPD-149) supports parametrized subject binding by:
...
**OPEN-1 — RESOLVED (operator-ratified 2026-05-29).** ep001_016 (flashback opening): **Mira IS present — `on_frame: true` is authoritative.** The `conference_room_flashback` is her re-experiencing the triggering anxiety event, and the rising arc (b17 peak) requires her on-frame at the onset; an empty room would be the weaker read. The empty `character_state: {}` is therefore the **defect**. **Resolution** (gold-standard ep_001 YAMLs stay immutable for round-trip integrity → applied in the `ep_001 v2` re-author): give ep001_016 a `character_state` for `mira_aoki` with `expression_dial ≈ 0.8, emotional: anxious`. This **also resolves OPEN-2** — b17's cited "0.8 → 0.9" antecedent then exists at b16. **Generator reconciliation rule (encode now):** when `relational_field.active_entities[].on_frame: true` but `character_state` is empty, FAIL-CLOSED (require a `character_state`) rather than silently emitting an empty subject.
