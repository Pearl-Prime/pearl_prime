# stillness_press ja_JP — Qwen / Tier 2 build (EXECUTED 2026-05-28)

**Status:** BOOKS BUILT via Qwen on Pearl Star (Tier 2). The prior "unreachable" stub was wrong —
Pearl Star is reachable over **Tailscale** (`$QWEN_BASE_URL` = `pearlstar.tail7fd910.ts.net:11434`,
`qwen2.5:14b`). The 4 book texts were translated by Qwen and assembled into ja EPUBs that pass
`validate_epub.py` (zero ERRORs). **Zero Claude-authored Japanese prose** — policy-compliant.

## What was built (this session)

- `books/text/book_*.ja.txt` — 4 books translated by `translate_book_ja.py` (Qwen `qwen2.5:14b`
  via Ollama `/api/generate`). Section headers mapped to EPUB-parseable Japanese markers
  (`第N章`, `教導縁起`, `深入修行`).
- `books/epub/book_*.ja.epub` — assembled by `build_ja_epubs.py` → `scripts/release/build_epub.py
  --language ja`, with the ComfyUI-rendered covers (downscaled to 1000×1600 to fit the brand1_deep
  raw-blob 1 MB cap). `dc:language=ja`. Validates with no ERRORs (CJK word-count + cover-ideal WARNs
  only — KDP accepts).

The original build plan (below) is retained for the manga-lettering + podcast ja continuation.

## Why ja prose is Qwen, not Claude
...
Claude (Pearl_Writer, Tier 1) wrote the **en_US** source deliverables. The Japanese versions are
generated by Qwen on Pearl Star — `python3 scripts/ci/audit_llm_callers.py` stays clean.
