...
line in `../lane06_operator_gate/OPERATOR_LOOK_GATE.md` — host ffmpeg is now fixed
(8.1.2 with libass) and the 3 pilot shortform items have **real, validated MP4s**.
...
1. `shortform_mp4_rendered/tt_anxiety_faceless.mp4` — TikTok/Reels/Shorts, anxiety, healthcare_staff persona
2. `shortform_mp4_rendered/tt_burnout_faceless.mp4` — TikTok/Reels/Shorts, burnout, corporate_managers persona
3. `shortform_mp4_rendered/yt_overthinking_faceless.mp4` — YouTube Shorts, overthinking, gen_z_students persona
...
4. `../lane07_pilot_packet/pilot_contact_sheets/publishable_pilot_sheet.jpg` (all 18 examples, one sheet)
5. `../lane04_static_carousel_rebuild/contact_sheets/static_carousel_publishable_sheet.jpg` (12 static + 3 carousels)
6. `../lane05_pearl_animator_rebuild/shortform_first_frame_contact_sheet.jpg` (3 shortform first frames, for before/after comparison against the new MP4s)

### Validation receipts (machine-checked, not a look judgment)

7. `shortform_mp4_render_validation_receipts.jsonl` — per-MP4: dims, fps, duration,
   sha256, full-decode-no-error, first-frame-vs-storyboard pixel diff
8. `../lane06_operator_gate/operator_look_scorecard.tsv` — prior automated pre-gate
